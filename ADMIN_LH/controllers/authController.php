...
$emailQuery = "SELECT * FROM useradminlh WHERE email=? LIMIT 1";
...
if (count($errors) === 0){
    ...
    $sql = "INSERT INTO useradminlh (username, email, verified, token, password) VALUES (?, ?, ?, ?, ?)";
...
if(count($errors) === 0){
    $sql = "SELECT * FROM useradminlh WHERE email=? OR username=? LIMIT 1";
$stmt = $conn->prepare($sql);
